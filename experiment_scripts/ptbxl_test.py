...
from experiment_code.model import (
    SeparateSourceClassifierResNet1D,
    SourceClassifierAppendResNet1D,
    SourcePredictMappingResNet1D,
    SourceCalcMappingResNet1D,
)
from experiment_code.model_evaluation import (
    accuracy_score,
...
parser = argparse.ArgumentParser(
    prog="PTBXL Experiment",
    description="This will run the PTBXL experiment with the given parameters",
)
...
train_dataset, test_dataset = data_factory.get_ptbxl(
    root="../../data/",
    subset=False,
    n_jobs=args.n_jobs,
)
...
def run_experiment(train_dataset, test_dataset, experiment_params, seed):
    ...
    model_dict = {
        "SeparateSourceClassifierResNet1D": SeparateSourceClassifierResNet1D(
            features=2, n_sources=experiment_params["n_sources"]
        ),
        "SeparateSourceBinaryClassifierResNet1D": SeparateSourceClassifierResNet1D(
            features=2,
            n_sources=experiment_params["n_sources"],
            heavyside=True,
            bias_classifier=False,
        ),
        "SourceClassifierAppendResNet1D": SourceClassifierAppendResNet1D(
            features=2,
            binary_dimensions=int(
                np.maximum(np.ceil(np.log2(experiment_params["n_sources"])), 1.0)
            ),
        ),
        "SourcePredictMappingResNet1D": SourcePredictMappingResNet1D(
            features=2,
            binary_dimensions=int(
                np.maximum(np.ceil(np.log2(experiment_params["n_sources"])), 1.0)
            ),
        ),
        "SourceCalcMappingPerBatchResNet1D": SourceCalcMappingResNet1D(
            features=2, n_sources=experiment_params["n_sources"]
        ),
    }
    ...
    params_key, dropout_key, key = jax.random.split(key=key, num=3)
    batch = next(iter(train_dl))

    try:
        variables = model.init(params_key, x=batch[0], source=batch[1], train=False)
        if experiment_params["model"] in [
            "SourceCalcMappingPerBatchResNet1D",
        ]:
        ...
        model_params = dict(
            ...
            **(
                ...
                if experiment_params["model"] == "SourceCalcMappingPerBatchResNet1D"
                else {}
            ),
        )
